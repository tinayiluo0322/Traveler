...
# Create a new dataframe summarizing claim rate by exposure bins
exposure_summary <- nova %>%
  mutate(exposure_bin = cut(exposure, breaks = "sturges")) %>%
  group_by(exposure_bin) %>%
  summarise(
    total_exposure = sum(exposure),
    number_of_claims = sum(clm),
    claim_rate = (number_of_claims / total_exposure) * 100
  ) %>%
  ungroup()

# Scatter plot with claim rate by exposure bins
ggplot(exposure_summary, aes(x = total_exposure, y = claim_rate)) + 
  geom_point() + 
  geom_smooth(method = "loess", se = FALSE) +
  ggtitle("Claim Rate by Exposure") +
  xlab("Total Exposure") +
  ylab("Claim Rate (%)") +
  theme_minimal()
...
Poisson Model Assessment (Failed)
